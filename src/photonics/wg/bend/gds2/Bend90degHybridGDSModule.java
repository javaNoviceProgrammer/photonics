...
public class Bend90degHybridGDSModule {
	...
	public Bend90degHybridGDSModule(
			double a,
			double b,
			double R
			) {
	...
	public double getAuxiliaryC(double theta) {
		if(0 <= theta && theta <= theta1)
			return 2*sqrt(theta) ;
		else if(theta1 < theta && theta <= PI/4.0)
	...
	public void createGDS(String filePath, boolean systemExit){
        ...
        try {
            ...
            File f ;
            if(filePath == null){
            	f = new File("bend90_hybrid_"+R+".gds");
            }
            else{
            	f = new File(filePath + File.separator + "bend90_hybrid_"+R+".gds");
            }
        	...
        	Struct topCell = new Struct("bend90_hybrid_"+R) ;
	...
	public static void main(String[] args) {
		Bend90degHybridGDSModule bend = new Bend90degHybridGDSModule(100, 2.49, 5) ;
		bend.setWidth(0.4);
